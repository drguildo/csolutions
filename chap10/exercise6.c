...
Bool flush, royalflush, straight, four, three;
...
void analyze_hand(int cards[NUM_CARDS][2])
{
	int num_consec, num_in_rank, lowest;
	int suit, rank, i;
	...
	/* check for royal flush. ugly. */
	if(flush) {
		Bool ten = FALSE, jack = FALSE, queen = FALSE, king = FALSE, ace = FALSE;

		for (i = 0; i < NUM_CARDS; i++) {
			switch(cards[i][0]) {
				case 8: ten = TRUE; break;
	...
	/* check for (aces-low) straight */
	lowest = 0;

	for (i = 0; i < NUM_CARDS; i++) {
		if ((cards[i][0] == 12 ? -1 : cards[i][0]) < (cards[lowest][0] == 12 ? -1 : cards[lowest][0])) {
			lowest = i;
		}
	}
	...
	scan:
	for (i = 0; i < NUM_CARDS; i++) {
		if ((cards[i][0] == 12 ? -1 : cards[i][0]) == ((cards[lowest][0] == 12 ? -1 : cards[lowest][0]) + 1)) {
			num_consec++;
			lowest = i;
			goto scan;
		}
